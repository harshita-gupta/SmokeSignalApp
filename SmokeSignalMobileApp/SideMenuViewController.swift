...
final class SideMenuViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let currentCenterController: UIViewController = (((self.parent as! MainViewController).frontViewController) as! UINavigationController).viewControllers[0] as UIViewController
        print(currentCenterController, terminator: "")
        
        // if on something other than about and about is selected, will switch to about
        if ((indexPath as NSIndexPath).row == 9 &&  ((currentCenterController is AboutUsViewController) == false )) {
            (self.parent as! MainViewController).setCenterControllerAndBarButton("centerViewAboutUs")
            return
        }
        
        // if on about and about is selected, will do nothing
        if ((indexPath as NSIndexPath).row == 9 &&  (currentCenterController is AboutUsViewController)) {
            return
        }
        ...
        if ((indexPath as NSIndexPath).row < 9 && (currentCenterController is AboutUsViewController) ) {
            ...
            ((self.parent as! MainViewController).frontViewController as! UINavigationController).viewControllers[0] = Singleton.sharedInstance.masterViewControllerReference
            ...
            DispatchQueue.main.async(execute: { () -> Void in
                ...
                print(self.giveSlugForSideMenuIndex((indexPath as NSIndexPath).row), terminator: "")
                Singleton.sharedInstance.masterViewControllerReference.refreshWithNewCatSort(self.giveSlugForSideMenuIndex((indexPath as NSIndexPath).row))
            })
            return
        }
        
        // therefore this only runs if on a section and another section is selected
        if ((indexPath as NSIndexPath).row != giveIndexForCatSlug(Singleton.sharedInstance.masterViewControllerReference.currentCategory.slug!) ) {
            Singleton.sharedInstance.posts = [NSMutableDictionary]()
            DispatchQueue.main.async(execute: { () -> Void in
                Singleton.sharedInstance.masterViewControllerReference.tableView.reloadData()
            })
            
            let catLabel : String = giveSlugForSideMenuIndex((indexPath as NSIndexPath).row)
    ...
    func giveSlugForSideMenuIndex (_ sideMenuIndex : Int) -> String {
        var catLabel : String
    ...
    func giveIndexForCatSlug (_ catSlug: String) -> Int {
    ...
    override func viewDidLoad() {
        ...
        tableFrame.origin = CGPoint(x: self.tableView.frame.origin.x, y: self.tableView.frame.origin.y + 20)
